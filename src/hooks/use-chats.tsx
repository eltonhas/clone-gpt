          ...
          [action.payload.chatId]: {
            ...
            messages: [
              ...(state.chats[action.payload.chatId]?.messages || []),
              {
                role: 'assistant',
                content:
                  'Ocorreu um erro ao tentar se comunicar com o servidor.Por favor verifique sua chave e tente novamente.',
              },
            ],
          },
        },
...
export function useChat(openAIKey: string) {
  ...
  const lastMessageSender =
    state.chats[state.selectedChat].messages[selectedChatMessageCount - 1]?.role

  useEffect(() => {
    async function getOpenAIResponse() {
      try {
        const { data: chat } = await api.post('/api/bot', {
          key: openAIKey,
        ...
        dispatch({
          type: 'update-assistant-response',
          payload: { chatId: state.selectedChat, chatToUpdate: chat },
        })
      } catch (error) {
        dispatch({
          type: 'update-assistant-response-error',
